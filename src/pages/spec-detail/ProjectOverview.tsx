...
//
// ProjectMeta 와 Spec 에 이미 담겨 오는 값만 쓴다. 추가 API 호출이 없다.
...
export function ProjectOverview({
  ...
  const { id, role, tryItBaseUrl } = meta;
  const { title, oasVersion, version, description, operations } = spec;

  const active = operations.filter((e) => !e.isDeleted);
  const deletedCount = operations.length - active.length;
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-8">
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-semibold text-fg-1">{title}</h2>
        {description && (
      ...
      <dl className="flex flex-col gap-3 text-sm">
        ...
        <Row label="스펙 버전">{version}</Row>
        <Row label="OpenAPI">{oasVersion}</Row>
