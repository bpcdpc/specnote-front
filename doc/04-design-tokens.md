...
| v0.7 | 2026.08.17 MON | 파비콘 완료 반영. 로고 절의 파비콘 항목을 **글자 path outline 변환**으로 갱신 — 텍스트 노드로 두면 Geist 미탑재 환경에서 대체 폰트로 그려져 16px 축소 시 형태가 무너진다.                                                                                                                                                                                                                                                                                                                                                               |
...
- **파비콘**은 검정 배경 + 흰 `{S}` 정방형 SVG다. 글자를 **path로 outline 변환**해 넣는다 —
  텍스트 노드로 두면 Geist가 없는 환경에서 대체 폰트로 그려져 중괄호 간격이 무너진다.
  파비콘은 브라우저 탭에서 16px로 축소되므로 그 차이가 형태 붕괴로 보인다.
