...
**URL 쿼리스트링도 상태의 출처다.** 알림 딥링크의 `?comment=`가 그렇다.
`CommentPanel`이 읽어 `CommentProvider`의 초기 하이라이트로 넘기고, 읽은 뒤에는
파라미터를 뗀다. 소비하고 버리는 값이라 어디에도 저장하지 않는다(`03-frontend-layouts`).
...
**쿼리스트링도 같은 함수로 거른다.** `?comment=`는 남이 준 URL에서 오는 값이라
path param보다 덜 믿을 이유는 있어도 더 믿을 이유는 없다.
...
- **뱃지 때문에 상시 조회가 필요하다.** 개수 전용 API가 없어 미확인 표시를 그리려면
  드롭다운이 닫혀 있어도 목록 쿼리가 돌아야 한다. "열 때만 조회"로는 뱃지를 못 만든다.
- **열려 있는 동안은 멈춘다.** 목록이 최신순(`desc`)이라 갱신되면 누르려던 항목이
  아래로 밀린다. 열 때 한 번 `refetch`하므로 최신 상태로 시작한다.
...
**화면에서 사라질 수 있는 컴포넌트에 쿼리를 두지 않는다.**

`NotificationDropdown`은 `DropdownMenuContent` 안에 산다. 항목을 클릭하면 팝업이 닫히면서
그 안의 컴포넌트가 통째로 언마운트된다. 그래서 알림 쿼리, 읽음 mutation, 클릭 핸들러는
전부 바깥(`UserMenu`)이 갖고 목록과 콜백만 내린다. 미확인 뱃지가 아바타에 붙는 것도
같은 사정이다 — 팝업이 닫혀 있을 때도 그려져야 한다.

**예외는 팝업이 열려 있는 동안에만 쓰이는 조회다.** `NotificationDropdown`이 프로젝트명을
붙이려고 읽는 `["projects"]`가 그렇다. 닫히면 필요 없고 이미 채워진 캐시라 즉시 히트한다.

**언마운트되는 경로에서는 `onSuccess` / `onError` 옵션을 쓰지 않는다.**

```tsx
// ❌ 이동하면서 UserMenu 가 사라지면 이 콜백은 실행되지 않는다
const readMutation = useMutation({
  mutationFn: markAsRead,
  onError: () => toast.add({ ... }),
});

// ✅ await 뒤 코드는 클로저라 컴포넌트가 사라져도 살아남는다
try {
...
`mutationFn` 자체는 계속 돌지만 **옵저버가 언마운트되면 옵션 콜백은 실행되지 않는다.**
대시보드에서 알림을 누르면 `AppLayout` → `SpecLayout`으로 레이아웃이 갈려 실제로 일어난다.
`CommentPanel`의 `confirmMove`, `confirmDelete`도 같은 형태다.

**순서는 부수효과가 먼저, 이동이 나중이다.** 이동이 먼저면 실패 토스트를 띄울 자리가
사라진다. 실패해도 이동은 진행한다 — 읽음 표시가 안 된 것과 알림 내용을 보는 것은 별개다.

`invalidateQueries`에는 `await`을 붙이지 않는다. 무효화는 `queryClient`가 하는 일이라
컴포넌트가 사라져도 계속 돌고, 기다리면 왕복 두 번 뒤에 화면이 바뀐다.
**예외는 그 재조회 결과에 의존하는 후속 동작이 있을 때다** — `CommentPanel`이
새 댓글로 스크롤하기 전에 `await`하는 것이 그렇다. DOM 노드가 아직 없으면
`scrollIntoView`가 빈 동작이 된다.
...
### 로그인 상태에 딸린 쿼리는 `enabled`로 막는다

`UserMenu`의 알림 쿼리가 그 예다. 훅은 조기 return 위에 있어야 하는데(`05-code-conventions`),
`logout()`이 `me`를 비운 직후 `RequireAuth`가 리다이렉트하기 전에 한 번 더 렌더된다.
`enabled: Boolean(me)`가 없으면 그 틈에 토큰 없는 요청이 나간다.
