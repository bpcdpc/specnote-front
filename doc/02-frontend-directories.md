...
| v0.8 | 2026.08.17 MON | 스펙 조회 재설계 반영. **`app/SpecAnchorContext.tsx` 신설** — 전역 Context 3번째. **`lib/api/endpoints.ts` 삭제** — `GET /api/endpoints/:id` 폐기로 이 파일에 남는 라우트가 없어졌다. `lib/api/` 도메인 분할 절의 `endpoints.ts` 문단 재작성. `SpecLayout`, `SpecDetailPage`, `SpecUpdateBanner` 트리 설명을 메타/스펙 두 쿼리 구조로 갱신. Context 배치 기준에 `SpecAnchorContext`와 그 근거 추가. **`EmptyState`를 `components/`에서 `pages/dashboard/`로 이동**하고 배치 예시에 그 판단을 남겼다. 미사용이던 `BackButton.tsx` 삭제.                                                  |
...
│   ├── SpecAnchorContext.tsx         # 프로젝트별 snapshotId 맵. 스펙 쿼리 키에 들어간다
...
│                                     #   메타/스펙 두 쿼리와 앵커 고정, 배너를 소유
│                                     #   Outlet context 로 { meta, spec, outdated } 전달
│
├── pages/                            # 화면 + 그 화면 전용 조각
│   ├── auth/
│   │   ├── LoginPage.tsx
...
│   │   ├── ProjectCard.tsx
│   │   ├── NewProjectCard.tsx
│   │   └── EmptyState.tsx            # 프로젝트 0건 안내 + 생성 버튼
│   │
...
│   │
│   ├── spec-detail/
│   │   ├── SpecDetailPage.tsx        # 3컬럼 본문. meta, spec 을 Outlet context 로 받는다
│   │   │                             #   엔드포인트 선택은 spec.operations.find(), 요청 없음
...
│   │   ├── SpecUpdateBanner.tsx      # 스펙 갱신 배너 (표시 전용, SpecLayout 이 소유)
...
│   │   ├── projects.ts               # 프로젝트 CRUD, 스펙 조회, 멤버, 스펙 커밋
...
- `EmptyState` — 처음에는 여러 화면이 쓸 것으로 보고 `components/`에 뒀으나
  실제 소비처가 대시보드 하나로 남았다 → `pages/dashboard/`.
  **"쓸 것 같다"가 아니라 "쓰고 있다"로 판단한다.**
...
`endpoints.ts`는 없다. 스펙 조회를 스냅샷 단위 단일 응답으로 재편하면서
`GET /api/endpoints/:id`가 폐기됐고, 그 파일에 남는 라우트가 없어졌다.
스펙 내용은 `projects.ts`의 `getSpec`이 한 번에 가져온다(`08-state-and-data`의 앵커 절).
경로가 `/endpoints`로 시작하는 나머지는 위 이유로 전부 `comments.ts`에 있다.
...
같은 기준이 Context에도 적용된다. `ThemeContext`, `AuthContext`, `SpecAnchorContext`는
라우터 위에 있어야 하므로 `app/`, `SpecPanelsContext`, `BearerTokenContext`,
`CommentContext`는 그 화면 트리 안에서만 사는 값이라 화면 폴더에 둔다.
Context라는 형태가 `app/` 배치의 근거가 되지 않는다.

**`SpecAnchorContext`가 `app/`인 이유는 앱 전체가 읽어서가 아니다.** 읽는 곳은
`SpecLayout`, `UserMenu`, `SpecJsonUrlField` 셋뿐이다. **화면을 떠나도 값이 살아야 해서**다 —
프로젝트를 옮겼다 돌아왔을 때 보던 스냅샷이 유지돼야 하는데, `pages/spec-detail/`에 두면
라우트를 벗어나는 순간 사라진다. 배치 기준이 "소비처 수"가 아니라 "수명"으로 갈리는
유일한 자리다.
