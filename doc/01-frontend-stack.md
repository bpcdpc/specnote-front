...
API 경로는 항상 상대경로 `/api`다. 배포는 NestJS 한 대가 프론트 정적 파일까지 서빙해
오리진이 같고, 개발에서는 이 프록시가 백엔드로 넘긴다. **환경변수 분기가 없다.**
`VITE_API_URL` 같은 것을 두지 않는다 — 배포 대상이 한 대뿐이라 분기할 대상이 없다.
...
| 4    | spec — `SpecLayout` 게이트, 엔드포인트 상세, 갱신 배너 무효화              | ✅   |
| 5    | comments — 목록, 작성, 수정, 삭제, 리액션, AI 요약, 이동                   | ☐    |

배관 5가 끝나면 `lib/mock.ts`를 제거한다.
