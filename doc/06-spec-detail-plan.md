...
| v0.3 | 2026.07.27 MON | 12단계 계획을 **`08-comments-plan`으로 분리.** 이 문서는 11단계(spec-detail 본문) 전용으로 범위를 고정한다. 우측 댓글 컬럼 항목에 후속 문서 포인터 추가.                                                                                             |

> **이 문서는 11단계 전용이다.** 12단계(댓글)는 `08-comments-plan`을 본다.
>
...
├── SpecPanelsContext.tsx     # 양쪽 패널 열림/접힘 · isWide 공유   ← 신설
├── BearerTokenContext.tsx    # 헤더 입력값을 TryItPanel까지 전달   ← 신설
├── BearerTokenInput.tsx      # 헤더 Bearer 입력
├── ProjectOverview.tsx       # 엔드포인트 미선택 시 중앙          ← 신설
...
├── buildExample.ts           # 스키마 → 요청 바디 초기값 조립      ← 신설
├── useTryIt.ts               # 요청 조립 · 전송 · 응답 상태        ← 신설
...
우측 댓글 컬럼은 11-2에서 빈 패널 자리표시만 둔다. **내용은 `08-comments-plan`(12단계).**
이 문서는 11단계 범위에서 닫는다.
...
- **엔드포인트 전환 시 `key={detail.id}`로 리마운트** — `useEffect`로 상태를 동기화하는
  것보다 안전하다. 펼침 상태, 입력값, 응답이 전부 한 번에 초기화된다.
- **`getMockEndpointDetail`의 참조 안정성** — 호출할 때마다 새 객체를 만들면
  재귀 트리의 memo가 전부 무효화된다. 목 데이터도 같은 참조를 돌려줘야 한다.
