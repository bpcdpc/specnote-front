...
| v0.2 | 2026.08.17 MON | **문서 충돌 표를 해소 기록으로 정리.** 네 건 모두 실제로 고쳤다 — `01-frontend-stack` v0.8, `08-state-and-data` v0.3, `01-overview` v0.4, `03-functional-requirements` v0.9. `10-api-specifications` 0-1절은 백엔드 관점에서 여전히 맞아 대상이 아니다. |
...
이 결정으로 다음 서술이 사실과 어긋났고, **2026.08.16에 네 건 모두 고쳤다.**

| 문서                         | 위치           | 고친 내용                                       | 버전 |
| ---------------------------- | -------------- | ----------------------------------------------- | ---- |
| `01-frontend-stack`          | 7절            | 프록시 기준으로 정정, 원안 폐기를 인용구로 명시 | v0.8 |
| `08-state-and-data`          | `client.ts` 절 | 같은 정정                                       | v0.3 |
| `01-overview`                | 10절, 12절     | 별도 App Service + 프록시, 오리진은 하나        | v0.4 |
| `03-functional-requirements` | NFR-3, NFR-4   | NFR-3 전면 재작성, NFR-4는 주체만 정정          | v0.9 |

전부 **결론이 아니라 주체**만 바꿨다. 상대경로 `/api`도, 환경변수 분기를 두지 않는 것도,
`/api` prefix 분리도 그대로다. `/api`를 넘겨주는 것이 NestJS에서 프론트 App Service의
Express로 옮겨갔을 뿐이다.

**NFR-4는 요구사항 자체가 유효했다.** "그 외 경로는 `index.html` 반환"은 주체가 바뀌어도
충족된다. 그래서 문장을 다시 쓰지 않고 전달 대상만 명시했다.

`10-api-specifications` 0-1절의 같은 문장도 백엔드 관점에서는 여전히 맞다 —
`/api` prefix는 바뀌지 않았다. 고칠 것이 없다.
